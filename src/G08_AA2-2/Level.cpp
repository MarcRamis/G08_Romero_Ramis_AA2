...
Level::Level(ELevelType _type)
{
	...
	case ELevelType::LEVEL1:
		...
		(map.at(0)->GetPlayer()->at(0).GetPosition()->y * SPRITE_RES) + SPRITE_HUD_HEIGHT + SPRITE_RES}, 
			Player::EPlayerType::PL1, map.at(0)->GetPlayer()->at(0).GetLives());
		initPlayer1Pos = *map.at(0)->GetPlayer()->at(0).GetPosition();

		AddPlayer({ (map.at(0)->GetPlayer()->at(1).GetPosition()->x * SPRITE_RES) + SPRITE_RES,
			(map.at(0)->GetPlayer()->at(1).GetPosition()->y * SPRITE_RES) + SPRITE_HUD_HEIGHT + SPRITE_RES}, 
			Player::EPlayerType::PL2, map.at(0)->GetPlayer()->at(1).GetLives());
		initPlayer2Pos = *map.at(0)->GetPlayer()->at(1).GetPosition();
	...
	case ELevelType::LEVEL2:
		...
		AddPlayer({ (map.at(1)->GetPlayer()->at(0).GetPosition()->x * SPRITE_RES) + SPRITE_RES,
			(map.at(1)->GetPlayer()->at(0).GetPosition()->y * SPRITE_RES) + SPRITE_HUD_HEIGHT + SPRITE_RES}, 
			Player::EPlayerType::PL1, map.at(1)->GetPlayer()->at(0).GetLives());
		initPlayer1Pos = *map.at(1)->GetPlayer()->at(0).GetPosition();

		AddPlayer({ (map.at(1)->GetPlayer()->at(1).GetPosition()->x * SPRITE_RES) + SPRITE_RES,
			(map.at(1)->GetPlayer()->at(1).GetPosition()->y * SPRITE_RES) + SPRITE_HUD_HEIGHT + SPRITE_RES }, 
			Player::EPlayerType::PL2, map.at(1)->GetPlayer()->at(1).GetLives());
		initPlayer2Pos = *map.at(1)->GetPlayer()->at(1).GetPosition();
					...
					for (int i = EXPLOSION_BLOCKS - 1; i >= 0; i--)
					{
						...
						for (; j < map.at(1)->GetWall()->size(); j++)
						{
							...
							if (Collisions::ExistCollision(*explosionBomb1.at(i)->GetPosition(),
								...
								(map.at(1)->GetWall()->at(j).GetPosition()->h * SPRITE_RES) + SPRITE_RES }) && !map.at(1)->GetWall()->at(j).destructed)
							{
								explosionBomb1.at(i)->exploding = false;
								if (i == 1)
									explosionBomb1.at(5)->exploding = false;
...
void Level::AddPlayer(VEC2 pos, Player::EPlayerType type, int lives)
{
	Player* p = new Player();
